(() => {
    ...
    function updateLaRudaSaleSplit() {
        ...
        const balances={Chiara:accountingBalance('Chiara')+recovery,Felipe:accountingBalance('Felipe')};
        const allocation=calculateApiarioIncomeAllocation(profit,balances);
    ...
    async function renderLaRuda(params) {
        ...
        const salesHistory=(data.sales||[]).length?`<section class="panel sales-history-panel"><div class="panel-header"><div><h2>Ventas registradas</h2><p>Separación entre recuperación de materiales y ganancia.</p></div></div><div class="stock-history-list">${data.sales.map(s=>`<article class="positive"><time>${formatDate(s.sale_date)}</time><strong>${integerQty(s.quantity)} un.</strong><div><span>${h(s.product_name)} · ${moneyARS(s.total_sale_ars)}</span><small>Recuperación material: ${moneyARS(s.material_cost_recovered_ars)} · Ganancia: Chiara ${moneyARS(s.chiara_profit_ars)} / Felipe ${moneyARS(s.felipe_profit_ars)}${s.buyer?` · ${h(s.buyer)}`:''}</small></div></article>`).join('')}</div></section>`:'';

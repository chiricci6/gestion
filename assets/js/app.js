(() => {
    ...
    async function renderTechnical(params) {
        ...
        const seasonFilter=seasonId||(hiveId?'':base.season?.id||''); const focusedHive=hiveId?(base.hives||[]).find(x=>String(x.id)===String(hiveId)):null;
        ...
        if(view==='inspections'){
            ...
            page=`<section class="technical-summary-grid"><article><small>Temporada</small><strong>${h(seasonFilter?((base.seasons||[]).find(x=>String(x.id)===String(seasonFilter))?.name||base.season?.name||'—'):'Todas')}</strong></article><article><small>${focusedHive?'Inspecciones de la colmena':'Inspecciones'}</small><strong>${integerQty(focusedHive?rows.length:(base.summary||{}).inspections||0)}</strong></article><article><small>Cosecha registrada</small><strong>${Number((base.summary||{}).harvest_kg||0).toLocaleString('es-AR',{maximumFractionDigits:2})} kg</strong></article></section><section class="technical-card-grid">${rows.length?rows.map(x=>`<button class="technical-record-card inspection-card technical-view-trigger" data-command="apiary-inspection-view" data-id="${x.id}"><div><span class="eyebrow">${formatDate(x.inspection_date)}</span><h3>${h(x.hive_name)}</h3></div><div class="inspection-quick-metrics"><span>Reina ${Number(x.queen_seen)?'✓':'—'}</span><span>Abejas ${integerQty(x.frames_bees||0)} c.</span><span>Miel ${h(capitalize(x.honey_reserve_status||'sin evaluar'))}</span><span>Polen ${h(capitalize(x.pollen_reserve_status||'sin evaluar'))}</span><span>Celdas ${integerQty(x.queen_cells||0)}</span></div><div class="technical-card-footer"><small>${x.file_count?`${integerQty(x.file_count)} archivos · `:''}${h(x.created_by_name||'')}</small><b>Ver detalle →</b></div></button>`).join(''):emptyState('◉',focusedHive?'Sin inspecciones para esta colmena':'Todavía no hay inspecciones','Registre una visita para comenzar el historial técnico.')}</section>`;

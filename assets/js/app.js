(() => {
    ...
    async function openLaRudaOrder(id) {
        ...
        showAppModal(`Pedido #${order.id} · ${order.customer_name}`, `<div class="la-ruda-order-head"><div><div class="la-ruda-customer-code"><small>NÚMERO DE CLIENTE</small><strong>${h(order.customer_code||'—')}</strong></div><span class="la-ruda-status status-${h(order.status)}">${h(laRudaStatusLabel(order.status))}</span><h3>${h(order.customer_name)}</h3><p>${formatDate(order.order_date)}${order.due_date?` · Entrega ${formatDate(order.due_date)}`:''}${order.customer_contact?` · ${h(order.customer_contact)}`:''}</p></div><button class="btn btn-small btn-secondary" data-command="la-ruda-order-edit" data-id="${order.id}">Editar datos</button></div>${order.notes?`<p class="la-ruda-order-notes">${nl2br(order.notes)}</p>`:''}
    ...
    async function renderLaRuda(params) {
        ...
        const orderCards=rows=>rows.length?`<div class="la-ruda-order-grid">${rows.map(o=>{const pct=Number(o.stage_count)?Math.round(Number(o.completed_count)*100/Number(o.stage_count)):0;return `<button class="la-ruda-order-card status-${h(o.status)}" type="button" data-command="la-ruda-order-open" data-id="${o.id}"><div class="la-ruda-order-card-top"><span class="la-ruda-status status-${h(o.status)}">${h(laRudaStatusLabel(o.status))}</span><strong>#${o.id}</strong></div><div class="la-ruda-customer-code compact"><small>CLIENTE</small><strong>${h(o.customer_code||'—')}</strong></div><h3>${h(o.customer_name)}</h3><p>${Number(o.item_count||0)} productos · ${moneyARS(o.total_amount)}</p><div class="production-progress"><span><i style="width:${pct}%"></i></span><small>${pct}% de etapas</small></div><footer><span>${formatDate(o.order_date)}</span><span>${o.due_date?`Entrega ${formatDate(o.due_date)}`:'Sin fecha de entrega'}</span></footer></button>`}).join('')}</div>`:emptyState('◆','Sin pedidos','No hay pedidos en esta vista.');

(() => {
    ...
    async function renderLaRuda(params) {
        ...
        const view=params.get('view')||'pedidos',summary=data.summary||{},seasons=data.seasons||[],activeSeason=seasons.find(x=>Number(x.is_active))||seasons[0]||null;
        ...
        const page=view==='stock'?stockCards:view==='fabricacion'?fabricationCards:view==='publicados'?publishedPage:view==='modelos'?modelsPage:view==='cupones'?couponsPage:view==='entregados'?`${deliveredFilters}${orderCards(delivered)}`:orderCards(active);
        shell({title:'Apiario La Ruda',subtitle:'Pedidos, fabricación, stock valorizado, modelos y ventas',active:'la-ruda',actions:'<button class="btn btn-secondary" data-command="la-ruda-product-new">+ Producto</button><button class="btn btn-secondary" data-command="la-ruda-production-new">+ Fabricar</button><button class="btn btn-primary" data-command="la-ruda-order-new">+ Nuevo pedido</button>',content:`<section class="la-ruda-summary la-ruda-summary-v17"><article><small>Pedidos activos</small><strong>${integerQty(summary.active)}</strong></article><article><small>En fabricación</small><strong>${integerQty(summary.fabricating)}</strong></article><article class="inventory-value-card"><small>Material valorizado en stock</small><strong>${moneyARS(summary.value_ars)}</strong><span>${moneyUSD(summary.value_usd)}</span></article><article><small>Modelos 3D</small><strong>${integerQty(summary.models_3d)}</strong></article></section><nav class="la-ruda-tabs"><a class="${view==='pedidos'?'active':''}" href="#/apiario-la-ruda?view=pedidos">Pedidos activos</a><a class="${view==='cupones'?'active':''}" href="#/apiario-la-ruda?view=cupones">Cupones</a><a class="${view==='fabricacion'?'active':''}" href="#/apiario-la-ruda?view=fabricacion">Fabricación</a><a class="${view==='stock'?'active':''}" href="#/apiario-la-ruda?view=stock">Stock y productos</a><a class="${view==='modelos'?'active':''}" href="#/apiario-la-ruda?view=modelos">Modelos 3D</a><a class="${view==='publicados'?'active':''}" href="#/apiario-la-ruda?view=publicados">Artículos publicados</a><a class="${view==='entregados'?'active':''}" href="#/apiario-la-ruda?view=entregados">Entregados</a></nav>${page}`});
    }

(() => {
    ...
    async function renderLaRuda(params) {
        ...
        const page=view==='stock'?stockCards:view==='fabricacion'?fabricationCards:view==='publicados'?publishedPage:view==='entregados'?orderCards(delivered):orderCards(active);
        shell({title:'Apiario La Ruda',subtitle:'Pedidos, fabricación, stock valorizado y ventas',active:'la-ruda',actions:'<button class="btn btn-secondary" data-command="la-ruda-product-new">+ Producto</button><button class="btn btn-secondary" data-command="la-ruda-production-new">+ Fabricar</button><button class="btn btn-primary" data-command="la-ruda-order-new">+ Nuevo pedido</button>',content:`<section class="la-ruda-hero panel"><div class="la-ruda-mark">◆</div><div><span class="eyebrow">APIARIO LA RUDA</span><h2>Fabricar, valorar el stock y vender</h2><p>Cada gramo utilizado queda relacionado con el costo real del producto.</p></div></section><section class="la-ruda-summary la-ruda-summary-v17"><article><small>Pedidos activos</small><strong>${integerQty(summary.active)}</strong></article><article><small>En fabricación</small><strong>${integerQty(summary.fabricating)}</strong></article><article class="inventory-value-card"><small>Material valorizado en stock</small><strong>${moneyARS(summary.value_ars)}</strong><span>${moneyUSD(summary.value_usd)}</span></article><article><small>Material dentro del stock</small><strong>${gramsLabel(summary.grams)}</strong></article></section><nav class="la-ruda-tabs"><a class="${view==='pedidos'?'active':''}" href="#/apiario-la-ruda?view=pedidos">Pedidos activos</a><a class="${view==='fabricacion'?'active':''}" href="#/apiario-la-ruda?view=fabricacion">Fabricación</a><a class="${view==='stock'?'active':''}" href="#/apiario-la-ruda?view=stock">Stock y productos</a><a class="${view==='publicados'?'active':''}" href="#/apiario-la-ruda?view=publicados">Artículos publicados</a><a class="${view==='entregados'?'active':''}" href="#/apiario-la-ruda?view=entregados">Entregados</a></nav>${page}`});
    }

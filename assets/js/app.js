(() => {
    ...
    async function renderMaterials(params) {
        ...
        const view = params.get('view') || ((status || category || q || edit) ? 'list' : 'categories');
        ...
        const pill = (value, label, count) => `<a class="material-state-card ${status === value && view==='list' ? 'active' : ''}" href="${listUrl(value ? {status:value} : {})}"><span>${value==='en_uso'?'↗':value==='reparacion'?'⌁':value==='disponible'?'✓':'◆'}</span><div><strong>${Number(count || 0)}</strong><small>${label}</small></div></a>`;
        const categoryCards = categories.length ? `<div class="material-category-grid">${categories.map(c=>`<a class="material-category-card" href="${listUrl({category:c.category})}"><div class="material-category-icon">${materialCategoryIcon(c.category)}</div><div class="material-category-title"><h3>${h(c.category)}</h3><strong>${Number(c.total||0)}</strong></div><div class="material-category-counts"><span><b>${Number(c.available||0)}</b> disponibles</span><span><b>${Number(c.in_use||0)}</b> en uso</span><span><b>${Number(c.repair||0)}</b> reparación</span></div><span class="material-category-arrow">Ver materiales →</span></a>`).join('')}</div>` : emptyState('◆','No hay materiales','Agregue el primer material para comenzar a organizar las categorías.');
        ...
        if (view === 'categories') {
            shell({title:'Materiales',subtitle:'Organizados por categorías, con disponibilidad y uso a simple vista',active:'materials',actions:'<a class="btn btn-secondary" href="#/materials?view=list">Ver lista completa</a><a class="btn btn-primary" href="#/materials?view=list">+ Agregar material</a>',content:`${summary}<section class="panel material-categories-panel"><div class="panel-header"><div><h2>Categorías de materiales</h2><p>Entre a una categoría para ver sus disponibles, usados y elementos en reparación.</p></div></div>${categoryCards}</section>`});
            return;
        }
        const categoryOptions = categories.map(c=>`<option value="${h(c.category)}">`).join('');
        shell({
            title: category ? `Materiales · ${category}` : 'Materiales', subtitle: 'Vista completa por lista', active: 'materials',
            actions:'<a class="btn btn-secondary" href="#/materials">Ver categorías</a>',
            content: `${summary}<div class="split-layout"><section class="panel sticky-panel"><div class="panel-header"><div><h2>${editing ? 'Editar material' : 'Agregar material'}</h2><p>Un registro por elemento</p></div></div><form data-form="material-save" data-material-form><input type="hidden" name="id" value="${editing?.id || ''}"><label class="field"><span>Nombre *</span><input type="text" name="name" required maxlength="160" value="${h(editing?.name || '')}" placeholder="Ej.: Alza mediana"></label><label class="field"><span>Categoría</span><input name="category" list="material-category-options" maxlength="100" value="${h(editing?.category || category || 'Otros materiales')}" placeholder="Ej.: Cuadros y marcos"><datalist id="material-category-options">${categoryOptions}</datalist></label><label class="field"><span>Estado</span><select name="status" data-material-status>${[['disponible','Disponible'],['en_uso','En uso'],['reparacion','En reparación']].map(([value,label]) => `<option value="${value}" ${(editing?.status || 'disponible') === value ? 'selected' : ''}>${label}</option>`).join('')}</select></label><label class="field" data-hive-field><span>Colmena</span><select name="hive_id"><option value="">Seleccione</option>${(data.hives || []).map(hive => `<option value="${hive.id}" ${Number(editing?.hive_id || 0) === Number(hive.id) ? 'selected' : ''}>${h(hive.name)}</option>`).join('')}</select></label><label class="field"><span>Notas</span><textarea name="notes" rows="3" placeholder="Detalle opcional">${h(editing?.notes || '')}</textarea></label><div class="form-actions"><button class="btn btn-primary" type="submit">${editing ? 'Guardar cambios' : '+ Agregar material'}</button>${editing ? '<a class="btn btn-ghost" href="#/materials?view=list">Cancelar</a>' : ''}</div></form></section>
                <section class="panel"><form class="filter-bar material-list-filter" data-form="material-filter"><input type="hidden" name="view" value="list"><label class="search-field"><span>⌕</span><input type="search" name="q" value="${h(q)}" placeholder="Buscar material, categoría o colmena"></label><label class="field compact-field"><span>Categoría</span><select name="category"><option value="">Todas</option>${categories.map(c=>`<option value="${h(c.category)}" ${category===c.category?'selected':''}>${h(c.category)}</option>`).join('')}</select></label><label class="field compact-field"><span>Estado</span><select name="status"><option value="">Todos</option>${[['disponible','Disponible'],['en_uso','En uso'],['reparacion','En reparación']].map(([v,l])=>`<option value="${v}" ${status===v?'selected':''}>${l}</option>`).join('')}</select></label><button class="btn btn-secondary" type="submit">Aplicar</button></form>${(data.materials || []).length ? `<div class="table-wrap"><table class="data-table"><thead><tr><th>Material</th><th>Categoría</th><th>Estado</th><th>Colmena</th><th>Notas</th><th></th></tr></thead><tbody>${data.materials.map(material => `<tr><td><strong>${h(material.name)}</strong></td><td><span class="material-category-chip">${materialCategoryIcon(material.category)} ${h(material.category)}</span></td><td><span class="badge material-${h(material.status)}">${{disponible:'Disponible',en_uso:'En uso',reparacion:'En reparación'}[material.status]}</span></td><td>${h(material.hive_name || '—')}</td><td class="cell-notes">${h(material.notes || '—')}</td><td class="row-actions"><a class="icon-button" href="#/materials?${new URLSearchParams({view:'list', ...(status ? {status} : {}), ...(category ? {category} : {}), ...(q ? {q} : {}), edit: material.id }).toString()}" title="Editar">✎</a><button class="icon-button danger" data-command="material-delete" data-id="${material.id}" title="Eliminar">×</button></td></tr>`).join('')}</tbody></table></div>` : emptyState('⬡', 'No hay materiales', 'No hay resultados para los filtros seleccionados.')}</section></div>`
        });
        updateMaterialForm();
    }
    ...
    async function openLaRudaOrder(id) {
        ...
        <section class="la-ruda-order-items"><div class="panel-header"><div><h3>Productos y etapas</h3><p>Cada parte queda marcada sin perder el historial del pedido.</p></div></div>${items.length?items.map(item=>{const pct=Number(item.stage_count)?Math.round(Number(item.completed_count)*100/Number(item.stage_count)):0;return `<article class="la-ruda-order-item ${item.manufacturing_completed_at?'manufactured':''}"><header><div><span class="product-line line-${h(item.business_line)}">${h(laRudaLineLabel(item.business_line))}</span><h3>${number3(item.quantity)} ${h(item.unit)} · ${h(item.product_name)}</h3>${item.manufacturing_completed_at?`<span class="manufactured-chip">✓ Fabricación ingresada al stock ${formatDateTime(item.manufacturing_completed_at)}</span>`:''}${item.notes?`<p>${h(item.notes)}</p>`:''}</div><div class="production-progress"><strong>${pct}%</strong><span><i style="width:${pct}%"></i></span></div></header><div class="production-stage-list">${(item.stages||[]).map(stage=>`<button type="button" class="production-stage ${Number(stage.completed)?'done':''}" data-command="la-ruda-stage-toggle" data-id="${stage.id}" data-order-id="${order.id}" data-completed="${Number(stage.completed)?0:1}" ${item.manufacturing_completed_at?'disabled':''}><span>${Number(stage.completed)?'✓':'○'}</span><div><strong>${h(stage.name)}</strong>${stage.completed_at?`<small>${formatDateTime(stage.completed_at)}${stage.completed_by_name?` · ${h(stage.completed_by_name)}`:''}</small>`:''}</div></button>`).join('')}</div>${!item.manufacturing_completed_at?`<button class="icon-button danger la-ruda-item-delete" type="button" data-command="la-ruda-item-delete" data-id="${item.id}" data-order-id="${order.id}" title="Quitar producto">×</button>`:''}</article>`}).join(''):'<p class="muted">Todavía no hay productos en este pedido.</p>'}</section>
    ...
    function openLaRudaStock(product) {
        showAppModal(`Stock · ${product.name}`, `<div class="stock-current"><small>Stock actual</small><strong>${number3(product.stock_quantity)} ${h(product.unit)}</strong></div><form data-form="la-ruda-stock-adjust"><input type="hidden" name="product_id" value="${product.id}"><label class="field"><span>Fecha</span><input type="date" name="movement_date" value="${today()}" required></label><label class="field"><span>Cantidad a sumar o restar</span><input type="number" name="quantity_change" step="0.001" placeholder="Ej.: 20 o -3" required><small>Use un número positivo para ingresar stock y negativo para descontar.</small></label><label class="field"><span>Motivo</span><textarea name="notes" rows="3"></textarea></label><button class="btn btn-primary">Actualizar stock</button></form>`, false);
    }

    function openLaRudaProductForm() {
        showAppModal('Nuevo producto', `<form data-form="la-ruda-product-save"><div class="form-grid two-columns"><label class="field full"><span>Nombre *</span><input name="name" required></label><label class="field"><span>Línea</span><select name="business_line"><option value="insumos">Insumos y producción 3D</option><option value="apiario">Material vivo y núcleos</option></select></label><label class="field"><span>Modo</span><select name="production_mode"><option value="stock">Se prepara desde stock</option><option value="por_pedido">Se fabrica por pedido</option></select></label><label class="field"><span>Unidad</span><input name="unit" value="unidad"></label><label class="field full"><span>Etapas, una por línea</span><textarea name="stages" rows="6" placeholder="Imprimir piezas\nArmado\nControl final"></textarea></label><label class="field full"><span>Notas</span><textarea name="notes"></textarea></label></div><button class="btn btn-primary">Crear producto</button></form>`, false);
    }

    async function openLaRudaProductHistory(id) {
        const data=await api('la_ruda_product_history',{params:{id}}),p=data.product;
        const movements=p.movements||[],made=p.manufacturing_history||[];
        showAppModal(`Producto · ${p.name}`, `<section class="product-history-hero"><div><span class="product-line line-${h(p.business_line)}">${h(laRudaLineLabel(p.business_line))}</span><h2>${h(p.name)}</h2><p>${p.production_mode==='por_pedido'?'Fabricación por pedido':'Preparación desde stock'} · ${Number(p.stage_count||0)} etapas</p></div><div class="stock-number"><strong>${number3(p.stock_quantity)}</strong><span>${h(p.unit)}</span></div></section><div class="product-history-actions"><button class="btn btn-secondary" data-command="la-ruda-stock-open" data-id="${p.id}">Mover stock</button><button class="btn btn-danger" data-command="la-ruda-product-delete" data-id="${p.id}">Eliminar producto</button></div><section class="product-history-section"><h3>Etapas habituales</h3><div class="product-stage-mini">${(p.stages||[]).map(x=>`<span>${h(x.name)}</span>`).join('')}</div></section><section class="product-history-section"><h3>Historial de stock</h3>${movements.length?`<div class="stock-history-list">${movements.map(m=>`<article class="${Number(m.quantity_change)>=0?'positive':'negative'}"><time>${formatDate(m.movement_date)}</time><strong>${Number(m.quantity_change)>=0?'+':''}${number3(m.quantity_change)} ${h(p.unit)}</strong><div><span>${h(m.notes||'Movimiento de stock')}</span><small>${m.order_id?`Pedido #${m.order_id} · ${h(m.customer_name||'')}`:''}${m.created_by_name?`${m.order_id?' · ':''}${h(m.created_by_name)}`:''}</small></div></article>`).join('')}</div>`:'<p class="muted">Todavía no hay movimientos.</p>'}</section><section class="product-history-section"><h3>Fabricaciones terminadas</h3>${made.length?`<div class="manufacturing-history-list">${made.map(x=>`<article><span>✓</span><div><strong>${number3(x.quantity)} ${h(p.unit)} · Pedido #${x.order_id}</strong><small>${h(x.customer_name)} · ${formatDateTime(x.manufacturing_completed_at)}${x.completed_by_name?` · ${h(x.completed_by_name)}`:''}</small></div></article>`).join('')}</div>`:'<p class="muted">Todavía no hay fabricaciones finalizadas.</p>'}</section>`, true);
    }
    ...
    async function renderLaRuda(params) {
        ...
        const fabricationOpen=(data.fabrication||[]).filter(x=>!x.manufacturing_completed_at);
        const fabricationDone=(data.fabrication||[]).filter(x=>x.manufacturing_completed_at);
        ...
        const stockCards=(data.products||[]).length?`<div class="la-ruda-product-grid">${data.products.map(p=>`<article class="la-ruda-product-card"><button class="la-ruda-product-open" type="button" data-command="la-ruda-product-open" data-id="${p.id}"><header><span class="product-line line-${h(p.business_line)}">${h(laRudaLineLabel(p.business_line))}</span><strong>${p.production_mode==='stock'?'Stock':'Por pedido'}</strong></header><h3>${h(p.name)}</h3><div class="stock-number"><strong>${number3(p.stock_quantity)}</strong><span>${h(p.unit)}</span></div><p>${Number(p.stage_count)} etapas de preparación</p><div class="product-stage-mini">${(p.stages||[]).map(x=>`<span>${h(x.name)}</span>`).join('')}</div><span class="product-history-link">Ver historial →</span></button><div class="la-ruda-product-actions"><button class="btn btn-small btn-secondary" data-command="la-ruda-stock-open" data-id="${p.id}">Mover stock</button><button class="icon-button danger" data-command="la-ruda-product-delete" data-id="${p.id}" title="Eliminar producto">×</button></div></article>`).join('')}</div>`:emptyState('◆','Sin productos','Agregue el primer producto.');
        const fabricationCards=`<section class="fabrication-board"><div class="panel-header"><div><h2>En fabricación</h2><p>Productos por pedido que todavía se están preparando.</p></div><strong>${fabricationOpen.length}</strong></div>${fabricationOpen.length?`<div class="fabrication-grid">${fabricationOpen.map(x=>{const pct=Number(x.stage_count)?Math.round(Number(x.completed_count)*100/Number(x.stage_count)):0;return `<article class="fabrication-card"><header><span class="la-ruda-status status-produccion">Fabricación</span><strong>${pct}%</strong></header><h3>${number3(x.quantity)} ${h(x.unit)} · ${h(x.product_name)}</h3><p>Pedido #${x.order_id} · ${h(x.customer_name)}</p><div class="production-progress"><span><i style="width:${pct}%"></i></span><small>${Number(x.completed_count||0)} de ${Number(x.stage_count||0)} etapas</small></div><footer><button class="btn btn-small btn-secondary" data-command="la-ruda-order-open" data-id="${x.order_id}">Abrir pedido</button><button class="btn btn-small btn-primary" data-command="la-ruda-manufacturing-complete" data-id="${x.order_item_id}" ${pct<100?'disabled title="Complete todas las etapas"':''}>Confirmar terminado</button></footer></article>`}).join('')}</div>`:emptyState('✓','No hay productos en fabricación','Los productos por pedido aparecerán aquí al agregarlos a un pedido.')}</section>${fabricationDone.length?`<section class="panel fabrication-history-panel"><div class="panel-header"><div><h2>Fabricación finalizada</h2><p>Ingresos automáticos al stock.</p></div></div><div class="manufacturing-history-list">${fabricationDone.slice(0,50).map(x=>`<article><span>✓</span><div><strong>${number3(x.quantity)} ${h(x.unit)} · ${h(x.product_name)}</strong><small>Pedido #${x.order_id} · ${h(x.customer_name)} · ${formatDateTime(x.manufacturing_completed_at)}${x.completed_by_name?` · ${h(x.completed_by_name)}`:''}</small></div></article>`).join('')}</div></section>`:''}`;
        const page=view==='stock'?stockCards:view==='fabricacion'?fabricationCards:view==='entregados'?orderCards(delivered):orderCards(active);
        shell({title:'Apiario La Ruda',subtitle:'Ventas, fabricación, productos y pedidos',active:'la-ruda',actions:'<button class="btn btn-secondary" data-command="la-ruda-product-new">+ Producto</button><button class="btn btn-primary" data-command="la-ruda-order-new">+ Nuevo pedido</button>',content:`<section class="la-ruda-hero panel"><div class="la-ruda-mark">◆</div><div><span class="eyebrow">APIARIO LA RUDA</span><h2>Del pedido a la entrega, parte por parte</h2><p>Núcleos, celdas reales e insumos 3D con stock e historial.</p></div></section><section class="la-ruda-summary"><article><small>Pedidos activos</small><strong>${Number(summary.active||0)}</strong></article><article><small>En fabricación</small><strong>${Number(summary.fabricating||0)}</strong></article><article><small>Listos para entregar</small><strong>${Number(summary.ready||0)}</strong></article><article><small>Entregados</small><strong>${Number(summary.delivered||0)}</strong></article></section><nav class="la-ruda-tabs"><a class="${view==='pedidos'?'active':''}" href="#/apiario-la-ruda?view=pedidos">Pedidos activos</a><a class="${view==='fabricacion'?'active':''}" href="#/apiario-la-ruda?view=fabricacion">Fabricación</a><a class="${view==='stock'?'active':''}" href="#/apiario-la-ruda?view=stock">Stock y productos</a><a class="${view==='entregados'?'active':''}" href="#/apiario-la-ruda?view=entregados">Entregados</a></nav>${page}`});
    }
    ...
    document.addEventListener('click', async event => {
        ...
        try {
            ...
            } else if (command === 'la-ruda-manufacturing-complete') {
                if(confirm('¿Confirmar que la fabricación terminó e ingresar esta cantidad al stock?')){const r=await api('la_ruda_manufacturing_complete',{method:'POST',data:{order_item_id:commandElement.dataset.id}});toast(r.message);await route();}

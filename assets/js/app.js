(() => {
    ...
    async function openLaRudaOrder(id) {
        ...
        <section class="la-ruda-order-photos"><div class="panel-header"><div><h3>Fotos del pedido</h3><p>Quedan guardadas con el pedido y siguen disponibles cuando pasa a Entregados.</p></div></div><form class="la-ruda-order-photo-form" data-form="la-ruda-order-photo-upload" enctype="multipart/form-data"><input type="hidden" name="order_id" value="${order.id}"><input type="file" name="photo" accept="image/jpeg,image/png,image/webp" required><input name="caption" maxlength="255" placeholder="Descripción opcional"><button class="btn btn-secondary">+ Agregar foto</button></form>${photos.length?`<div class="la-ruda-order-photo-grid">${photos.map(photo=>`<article><button type="button" class="la-ruda-order-photo-open" data-command="open-file" data-file-type="la_ruda_order_photo" data-id="${photo.id}" data-name="${h(photo.original_name)}"><img data-protected-image data-file-type="la_ruda_order_photo" data-id="${photo.id}" alt="${h(photo.caption||photo.original_name)}"><strong>${h(photo.caption||photo.original_name)}</strong></button><button type="button" class="icon-button danger" data-command="la-ruda-order-photo-delete" data-id="${photo.id}" data-order-id="${order.id}" title="Eliminar foto">×</button></article>`).join('')}</div>`:'<p class="muted empty-line">Todavía no hay fotos guardadas.</p>'}</section>
    ...
    async function renderLaRuda(params) {
        ...
        const orderCards=rows=>rows.length?`<div class="la-ruda-order-grid">${rows.map(o=>{const pct=Number(o.stage_count)?Math.round(Number(o.completed_count)*100/Number(o.stage_count)):0;return `<button class="la-ruda-order-card status-${h(o.status)}" type="button" data-command="la-ruda-order-open" data-id="${o.id}"><div class="la-ruda-order-card-top"><span class="la-ruda-status status-${h(o.status)}">${h(laRudaStatusLabel(o.status))}</span><strong>#${h(o.customer_code||o.id)}</strong></div><h3>${h(o.customer_name)}</h3><p>${Number(o.item_count||0)} productos · ${moneyARS(o.total_amount)}${Number(o.photo_count||0)?` · ${integerQty(o.photo_count)} fotos`:''}</p><div class="production-progress"><span><i style="width:${pct}%"></i></span><small>${pct}% de etapas</small></div><footer><span>${formatDate(o.order_date)}</span><span>${o.status==='entregado'&&o.delivered_at?`Entregado ${formatDate(o.delivered_at)}`:(o.due_date?`Entrega ${formatDate(o.due_date)}`:'Sin fecha de entrega')}</span></footer></button>`}).join('')}</div>`:emptyState('◆','Sin pedidos','No hay pedidos en esta vista.');
        ...
        const page=view==='stock'?stockCards:view==='fabricacion'?fabricationCards:view==='publicados'?publishedPage:view==='modelos'?modelsPage:view==='entregados'?`${deliveredFilters}${orderCards(delivered)}`:orderCards(active);
    ...
    document.addEventListener('submit', async event => {
        ...
        try {
            if (type === 'login') {
                ...
                const orderId=Number(form.elements.order_id.value);const result=await api('la_ruda_order_photo_upload',{method:'POST',formData:new FormData(form)});invalidateImageCache('la_ruda_order_photo');toast(result.message||'Foto guardada');await openLaRudaOrder(orderId);
    ...
    document.addEventListener('click', async event => {
        ...
        try {
            if (command === 'app-switch-toggle') {
                ...
                if(confirm('¿Eliminar esta foto del pedido?')){await api('la_ruda_order_photo_delete',{method:'POST',data:{id:commandElement.dataset.id}});invalidateImageCache('la_ruda_order_photo');toast('Foto eliminada');await openLaRudaOrder(Number(commandElement.dataset.orderId));}

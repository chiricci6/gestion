(() => {
            ...
            <div class="app-shell">
                <aside class="sidebar" id="sidebar">
                    <a class="brand" href="#/dashboard">
                        ...
                        <span><strong>Gestión Apícola</strong><small>Acceso privado</small></span>
                    </a>
                    <nav class="nav-menu">
    ...
    async function renderActivities(params) {
        ...
        shell({
            ...
            content: `<form class="filter-bar" data-form="activity-filter"><label class="search-field"><span>⌕</span><input type="search" name="q" value="${h(filters.q)}" placeholder="Buscar actividad"></label><select name="hive_id"><option value="">Todas las colmenas</option>${(data.hives || []).map(item => `<option value="${item.id}" ${String(item.id) === String(filters.hive_id) ? 'selected' : ''}>${h(item.name)}</option>`).join('')}</select><select name="label_id"><option value="">Todas las etiquetas</option>${(data.labels || []).map(item => `<option value="${item.id}" ${String(item.id) === String(filters.label_id) ? 'selected' : ''}>${h(item.name)}</option>`).join('')}</select><button class="btn btn-secondary" type="submit">Filtrar</button><a class="btn btn-ghost" href="#/activities">Limpiar</a></form>
                <section class="kanban-board" data-kanban-board>${(data.statuses || []).map(status => { const cards = (data.activities || []).filter(item => Number(item.status_id) === Number(status.id)); return `<article class="kanban-column"><div class="kanban-header" style="--status-color:${h(status.color)}"><div><span></span><h2>${h(status.name)}</h2></div><strong>${cards.length}</strong></div><div class="kanban-list" data-status-id="${status.id}">${cards.map(activity => `<article class="kanban-card" draggable="true" data-activity-id="${activity.id}"><a href="#/activity-edit/${activity.id}"><div class="kanban-card-top">${activity.label_name ? `<span class="activity-label" style="--label-color:${h(activity.label_color)}">${h(activity.label_name)}</span>` : '<span></span>'}<span class="priority priority-${h(activity.priority)}">${capitalize(activity.priority)}</span></div><h3>${h(activity.title)}</h3>${activity.description ? `<p>${h(activity.description)}</p>` : ''}<div class="kanban-card-meta"><span>⬡ ${h(activity.hive_name || 'Sin colmena')}</span><span>◷ ${formatDate(activity.due_date)}</span></div>${activity.responsible ? `<small>Responsable: ${h(activity.responsible)}</small>` : ''}</a></article>`).join('')}</div></article>`; }).join('')}</section>`
        });
        initKanban();
    }
    ...
    function initKanban() {
        ...
        board.querySelectorAll('.kanban-list').forEach(list => {
            ...
            list.addEventListener('drop', async event => {
                ...
                if (!dragged) return;
                const payload = { activity_id: Number(dragged.dataset.activityId), status_id: Number(list.dataset.statusId), ordered_ids: [...list.querySelectorAll('.kanban-card')].map(card => Number(card.dataset.activityId)) };
    ...
    async function renderPurchases(params) {
        loading('Cargando compras pendientes…');
    ...
    async function renderAccounting(params) {
        ...
        shell({
            ...
            content: `<section class="balance-strip accounting-balances"><div class="balance-strip-title"><span>Saldo histórico por persona</span><small>Siempre muestra toda la existencia del proyecto</small></div>${(data.balances || []).map(balance => `<div class="person-balance ${Number(balance.balance_ars) < 0 ? 'negative' : 'positive'}"><strong>${h(balance.name)}</strong><span>${moneyARS(balance.balance_ars)}</span><small>${moneyUSD(balance.balance_usd)}</small></div>`).join('')}</section>
                ...
                <section class="accounting-summary-grid"><article><small>Ingresos del resultado</small><strong class="income-text">${moneyARS(summary.income_ars)}</strong><span>${moneyUSD(summary.income_usd)}</span></article><article><small>Egresos del resultado</small><strong class="expense-text">${moneyARS(summary.expense_ars)}</strong><span>${moneyUSD(summary.expense_usd)}</span></article><article><small>Balance del resultado</small><strong>${moneyARS(Number(summary.income_ars || 0)-Number(summary.expense_ars || 0))}</strong><span>${moneyUSD(Number(summary.income_usd || 0)-Number(summary.expense_usd || 0))}</span></article><article><small>Movimientos encontrados</small><strong>${Number(summary.total || 0)}</strong><span>registros</span></article></section>

(() => {
    ...
    const activityLabelName = value => {
        const text = String(value || '').trim();
        const key = text.toLocaleLowerCase('es-AR');
        if (['comunidad','sanidad','sanitario','control sanitario'].includes(key)) return 'Control sanitario';
        return text;
    };
    const activityLabels = rows => {
        const seen = new Set();
        return (rows || []).map(row => ({ ...row, name: activityLabelName(row.name) })).filter(row => {
            const key = String(row.name || '').toLocaleLowerCase('es-AR');
            if (!key || seen.has(key)) return false;
            seen.add(key); return true;
        });
    };
    const pendingActivityColor = activity => activityLabelName(activity?.label_name) === 'Control sanitario' ? '#4f8b62' : (activity?.label_color || '#b58b2b');

    const accountingPerson = name => (state.accountingPeople || []).find(p => String(p.name || '').trim().toLocaleLowerCase('es-AR') === String(name).toLocaleLowerCase('es-AR'));
    const accountingBalance = name => Number((state.accountingBalances || []).find(b => String(b.name || '').trim().toLocaleLowerCase('es-AR') === String(name).toLocaleLowerCase('es-AR'))?.balance_ars || 0);
    function calculateApiarioIncomeAllocation(value, balanceOverrides = null) {
        const amount = Math.max(0, Number(value) || 0);
        const chiara = accountingPerson('Chiara'), felipe = accountingPerson('Felipe');
        if (!chiara || !felipe || amount <= 0) return [];
        const balances = balanceOverrides || { Chiara: accountingBalance('Chiara'), Felipe: accountingBalance('Felipe') };
        const debtC = Math.max(0, -balances.Chiara), debtF = Math.max(0, -balances.Felipe);
        let remain = amount, c = 0, f = 0;
        const totalDebt = debtC + debtF;
        if (remain > 0 && totalDebt > 0) {
            const cover = Math.min(remain, totalDebt);
            if (debtC > 0 && debtF > 0) { c += cover * debtC / totalDebt; f += cover * debtF / totalDebt; }
            else if (debtC > 0) c += cover;
            else f += cover;
            remain -= cover;
        }
        if (remain > 0) { c += remain / 2; f += remain / 2; }
        c = Math.round(c * 100) / 100; f = Math.round((amount - c) * 100) / 100;
        return [
            { person_id:chiara.id, name:'Chiara', balance:balances.Chiara, amount:c, percent:amount ? c / amount * 100 : 0 },
            { person_id:felipe.id, name:'Felipe', balance:balances.Felipe, amount:f, percent:amount ? f / amount * 100 : 0 }
        ];
    }
    ...
    async function renderHive(id) {
        ...
        const [data,inspectionData,healthData,feedingData,harvestData] = await Promise.all([
            api('hive', { params: { id } }),
            api('apiary_inspections_list'),
            api('apiary_health_list'),
            api('apiary_feedings_list'),
            api('apiary_harvests_list')
        ]);
        const hive = data.hive;
        const inspections=(inspectionData.inspections||[]).filter(x=>technicalRecordHasHive(x,id));
        const healthRecords=(healthData.records||[]).filter(x=>technicalRecordHasHive(x,id));
        const feedings=(feedingData.feedings||[]).filter(x=>technicalRecordHasHive(x,id));
        const harvests=(harvestData.harvests||[]).filter(x=>technicalRecordHasHive(x,id));
        state.apiaryTechnical={...(state.apiaryTechnical||{}),inspections,records:healthRecords,feedings,harvests};
        const technicalHistory=[
            ...inspections.map(row=>({kind:'inspection',date:row.inspection_date||'',row})),
            ...healthRecords.map(row=>({kind:'health',date:row.record_date||'',row})),
            ...feedings.map(row=>({kind:'feeding',date:row.feeding_date||'',row})),
            ...harvests.map(row=>({kind:'harvest',date:row.harvest_date||'',row}))
        ].sort((a,b)=>String(b.date).localeCompare(String(a.date))||Number(b.row.id||0)-Number(a.row.id||0));
        ...
        const pendingActivities = [...openActivities].sort((a,b) => {
        ...
        shell({
            ...
            content: `
                ...
                ${(() => { const t=data.technical||{}, si=t.season||{}, li=t.last_inspection||{}; return `<section class="hive-technical-strip panel hive-management-center"><div class="hive-management-create create-first"><small>REGISTRAR AHORA</small><div><button class="btn btn-small btn-primary" type="button" data-command="apiary-inspection-new" data-hive-id="${hive.id}">+ Inspección</button><button class="btn btn-small btn-secondary" type="button" data-command="apiary-health-new" data-hive-id="${hive.id}">+ Sanidad</button><button class="btn btn-small btn-secondary" type="button" data-command="apiary-feeding-new" data-hive-id="${hive.id}">+ Alimentación</button></div></div><div class="hive-technical-title"><div class="hive-season-heading"><span class="eyebrow">MANEJO</span>${si.name?'<span class="season-context-chip active">TEMPORADA ACTIVA</span>':''}</div><h2>${h(si.name||'Sin temporada activa')}</h2><p>${li.inspection_date?`Última inspección: ${formatDate(li.inspection_date)}`:'Todavía no hay inspecciones registradas.'}</p>${(t.timeline||[])[0]?`<small class="hive-last-action">Último registro: ${h((t.timeline||[])[0].title)} · ${formatDate((t.timeline||[])[0].event_date)}</small>`:''}${pendingActivities.length?`<div class="hive-next-actions"><small>PRÓXIMOS PENDIENTES</small><div>${pendingActivities.slice(0,3).map(a=>`<span class="hive-next-action ${a.due_date&&a.due_date<today()?'is-overdue':''}" style="--pending-color:${h(pendingActivityColor(a))}"><b>${h(activityLabelName(a.label_name)||'Actividad')}</b><strong>${h(a.title)}</strong><em>${a.due_date?formatDate(a.due_date):'Sin fecha'}</em></span>`).join('')}</div></div>`:''}</div><div class="hive-technical-metrics"><article><small>Cosecha</small><strong>${Number(t.harvest_kg||0).toLocaleString('es-AR',{maximumFractionDigits:2})} kg</strong></article><article><small>Sanidad</small><strong>${integerQty(t.health_count||0)}</strong></article><article><small>Alimentaciones</small><strong>${integerQty(t.feeding_count||0)}</strong></article></div>${renderHiveTechnicalHistory(technicalHistory,hive,cover)}}</section>`; })()}
                ...
                <div class="detail-grid hive-detail-grid">
                    <section class="panel span-2 animated-panel hive-observations-panel"><div class="panel-header"><div><h2>Observaciones</h2><p>Puede agregar todas las que necesite</p></div></div>
                    ...
                    <section class="panel animated-panel hive-materials-panel"><div class="panel-header"><div><h2>Materiales en uso</h2><p>Asignados a esta colmena</p></div><a class="btn btn-small btn-secondary" href="#/materials">Administrar</a></div>${(data.materials || []).length ? `<div class="simple-list">${data.materials.map(material => `<div><span>⬡</span><strong>${h(material.name)}</strong></div>`).join('')}</div>` : '<p class="muted empty-line">No tiene materiales asignados.</p>'}</section>
                    <section class="panel span-2 animated-panel hive-activities-panel"><div class="panel-header"><div><h2>Actividades abiertas</h2><p>Trabajo pendiente en esta colmena</p></div><button class="btn btn-small btn-primary" type="button" data-command="activity-open" data-hive-id="${hive.id}">+ Actividad</button></div>${openActivities.length ? `<div class="activity-preview-grid hive-activity-preview-grid">${openActivities.map(activity => `<button type="button" class="activity-preview-card priority-preview-${h(activity.priority)}" data-command="activity-open" data-id="${activity.id}"><div class="activity-preview-top"><span class="activity-status-chip" style="--activity-status:${h(activity.status_color || '#78906f')}"><i></i>${h(activity.status_name || 'Pendiente')}</span><span class="priority priority-${h(activity.priority)}">${capitalize(activity.priority)}</span></div><h3>${h(activity.title)}</h3><div class="activity-preview-meta"><span>Colmena: ${h(hive.name)}</span><time>◷ ${formatDate(activity.due_date)}</time></div>${activity.label_name ? `<span class="activity-label activity-preview-label" style="--label-color:${h(activity.label_color || '#64748b')}">${h(activityLabelName(activity.label_name))}</span>` : '<span class="activity-label activity-preview-label label-empty">Sin etiqueta</span>'}<span class="activity-preview-arrow">Ver actividad →</span></button>`).join('')}</div>` : '<p class="muted empty-line">No hay actividades abiertas.</p>'}</section>
                    ...
                    <section class="panel span-3 animated-panel hive-photos-panel"><div class="panel-header"><div><h2>Fotografías y archivos</h2><p>Seleccione una fotografía como banner de la colmena</p></div></div>
    ...
    async function openActivityView(id = 0) {
        const data = await api('activity', { params: { id } });
        const item = data.activity;
        if (!item) return;
        const attachments = data.attachments || [];
        const logs = data.logs || [];
        const activityLabel = activityLabelName(item.label_name || (data.labels || []).find(label => Number(label.id) === Number(item.label_id || 0))?.name || '');
        showAppModal(`Actividad · ${item.title || 'Sin título'}`, `<div class="technical-view apiculture-activity-view">
            <section class="technical-view-hero activity-view-hero"><div class="technical-view-icon">✓</div><div><span class="eyebrow">ACTIVIDAD</span><h2>${h(item.title || 'Sin título')}</h2><p>${h(item.status_name || 'Pendiente')} · ${h(item.hive_name || 'Sin colmena')}</p></div><div class="technical-view-actions"><button class="btn btn-secondary" type="button" data-command="activity-edit" data-id="${item.id}">Editar</button><button class="btn btn-ghost" type="button" data-command="modal-close">Cerrar</button></div></section>
            <section class="technical-view-grid"><div class="technical-view-field"><small>Responsable</small><strong>${h(item.responsible || 'Sin asignar')}</strong></div><div class="technical-view-field"><small>Fecha prevista</small><strong>${formatDate(item.due_date)}</strong></div><div class="technical-view-field"><small>Prioridad</small><strong>${capitalize(item.priority || 'normal')}</strong></div><div class="technical-view-field"><small>Etiqueta</small><strong>${h(activityLabel || 'Sin etiqueta')}</strong></div></section>
            ${item.description ? `<section class="technical-view-notes"><small>DESCRIPCIÓN</small><p>${nl2br(item.description)}</p></section>` : ''}
            ${attachments.length ? `<section class="technical-view-section"><div class="technical-view-section-title"><span>▤</span><div><small>ARCHIVOS</small><h3>Adjuntos</h3></div></div><div class="technical-view-files">${attachments.map(file => `<button type="button" class="technical-view-file" data-command="open-file" data-file-type="activity" data-id="${file.id}" data-name="${h(file.original_name)}">${String(file.mime_type).startsWith('image/') ? `<img data-protected-image data-file-type="activity" data-id="${file.id}" alt="${h(file.original_name)}">` : '<span class="technical-view-pdf">PDF</span>'}<strong>${h(file.original_name)}</strong><small>Abrir archivo</small></button>`).join('')}</div></section>` : ''}
            ${logs.length ? `<section class="technical-view-section"><div class="technical-view-section-title"><span>↺</span><div><small>SEGUIMIENTO</small><h3>Historial</h3></div></div><div class="history-list">${logs.map(log => `<div><strong>${h(log.action)}</strong><small>${formatDateTime(log.created_at)}</small></div>`).join('')}</div></section>` : ''}
        </div>`);
    }
    ...
    async function openLaRudaOrder(id, editMode = false) {
        const data=await api('la_ruda_order',{params:{id}}),order=data.order,products=state.laRudaData?.products||[];
        const items=order.items||[],photos=order.photos||[];
        const stageHtml=item=>(item.stages||[]).map(stage=>editMode
            ...
            : `<div class="production-stage readonly ${Number(stage.completed)?'done':''}"><span>${Number(stage.completed)?'✓':'○'}</span><div><strong>${h(stage.name)}</strong>${stage.completed_at?`<small>${formatDateTime(stage.completed_at)}${stage.completed_by_name?` · ${h(stage.completed_by_name)}`:''}</small>`:''}</div></div>`
        ).join('');
        const itemsHtml=items.length?items.map(item=>{
            const pct=Number(item.stage_count)?Math.round(Number(item.completed_count)*100/Number(item.stage_count)):0;
            return `<article class="la-ruda-order-item ${editMode?'is-editing':''}"><header><div><span class="product-line">${h(item.category_name||'Producto')}</span><h3>${integerQty(item.quantity)} ${h(item.unit)} · ${h(item.product_name)}</h3>${item.notes?`<p>${h(item.notes)}</p>`:''}</div><div class="production-progress"><strong>${pct}%</strong><span><i style="width:${pct}%"></i></span></div></header><div class="production-stage-list">${stageHtml(item)}</div>${editMode?`<button class="icon-button danger la-ruda-item-delete" type="button" data-command="la-ruda-item-delete" data-id="${item.id}" data-order-id="${order.id}" title="Quitar producto">×</button>`:''}</article>`;
        }).join(''):'<p class="muted">Todavía no hay productos en este pedido.</p>';
        const photoForm=editMode?`<form class="la-ruda-order-photo-form" data-form="la-ruda-order-photo-upload" enctype="multipart/form-data"><input type="hidden" name="order_id" value="${order.id}"><input type="file" name="photo" accept="image/jpeg,image/png,image/webp" required><input name="caption" maxlength="255" placeholder="Descripción opcional"><button class="btn btn-secondary">+ Agregar foto</button></form>`:'';
        const photosHtml=photos.length?`<div class="la-ruda-order-photo-grid">${photos.map(photo=>`<article><button type="button" class="la-ruda-order-photo-open" data-command="open-file" data-file-type="la_ruda_order_photo" data-id="${photo.id}" data-name="${h(photo.original_name)}"><img data-protected-image data-file-type="la_ruda_order_photo" data-id="${photo.id}" alt="${h(photo.caption||photo.original_name)}"><strong>${h(photo.caption||photo.original_name)}</strong></button>${editMode?`<button type="button" class="icon-button danger" data-command="la-ruda-order-photo-delete" data-id="${photo.id}" data-order-id="${order.id}" title="Eliminar foto">×</button>`:''}</article>`).join('')}</div>`:'<p class="muted empty-line">Todavía no hay fotos guardadas.</p>';
        const editTools=editMode?`
            <div class="la-ruda-edit-banner"><strong>Modo edición</strong><span>Ahora sí puede cambiar estados, etapas, productos y fotos.</span></div>
        ...
        const addItemForm=editMode?`<form class="la-ruda-add-item" data-form="la-ruda-item-save"><input type="hidden" name="order_id" value="${order.id}"><label class="field"><span>Producto</span><select name="product_id" required><option value="">Seleccionar</option>${products.map(p=>`<option value="${p.id}">${h(p.name)} · stock ${integerQty(p.stock_quantity)}</option>`).join('')}</select></label><label class="field"><span>Cantidad</span><input type="number" name="quantity" min="1" step="1" value="1" required></label><label class="field"><span>Precio unitario</span><input type="number" name="unit_price" min="0" step="0.01" value="0"></label><label class="field grow"><span>Detalle</span><input name="notes" placeholder="Opcional"></label><button class="btn btn-primary">Agregar</button></form>`:'';
        const topAction=editMode
            ? `<div class="la-ruda-order-head-actions"><button class="btn btn-small btn-ghost" data-command="la-ruda-order-view-mode" data-id="${order.id}">Terminar edición</button><button class="btn btn-small btn-secondary" data-command="la-ruda-order-edit" data-id="${order.id}">Editar datos</button></div>`
            : `<button class="btn btn-small btn-secondary" data-command="la-ruda-order-edit-mode" data-id="${order.id}">Editar pedido</button>`;
        showAppModal(`Pedido #${order.customer_code||order.id} · ${order.customer_name}`, `<div class="la-ruda-order-head"><div><span class="la-ruda-status status-${h(order.status)}">${h(laRudaStatusLabel(order.status))}</span><h3>${h(order.customer_name)}</h3><p>${formatDate(order.order_date)}${order.due_date?` · Entrega ${formatDate(order.due_date)}`:''}${order.customer_contact?` · ${h(order.customer_contact)}`:''}${order.season_id?` · Temporada ${h((state.laRudaData?.seasons||[]).find(x=>String(x.id)===String(order.season_id))?.name||'')}`:''}</p><div class="la-ruda-order-code"><span>Número cliente / cupón</span><strong>${h(order.customer_code||order.id)}</strong></div></div>${topAction}</div>${order.notes?`<p class="la-ruda-order-notes">${nl2br(order.notes)}</p>`:''}
        ${editTools}
        <section class="la-ruda-order-items"><div class="panel-header"><div><h3>Productos del pedido</h3><p>${editMode?'Puede actualizar las etapas de preparación.':'Vista de consulta. Para modificar el pedido use Editar pedido.'}</p></div></div>${itemsHtml}</section>
        <section class="la-ruda-order-photos"><div class="panel-header"><div><h3>Fotos del pedido</h3><p>Quedan guardadas con el pedido y siguen disponibles cuando pasa a Entregados.</p></div></div>${photoForm}${photosHtml}</section>
        ${addItemForm}
        ${editMode?`<div class="form-actions"><button type="button" class="btn btn-danger" data-command="la-ruda-order-delete" data-id="${order.id}">Eliminar pedido</button></div>`:''}`, true);
    }
    ...
    async function openLaRudaSale(product) {
        const avg=Number(product.average_cost_ars||0);
        showAppModal(`Registrar venta · ${product.name}`, `<section class="sale-product-resume">${productPhoto(product,'sale-product-photo')}<div><small>Stock disponible</small><strong>${integerQty(product.stock_quantity)} unidades</strong><span>Al registrar la venta se descuenta únicamente el stock.</span></div></section><form data-form="la-ruda-sale-save"><input type="hidden" name="product_id" value="${product.id}"><div class="form-grid two-columns"><label class="field"><span>Fecha</span><input type="date" name="sale_date" value="${today()}" required></label><label class="field"><span>Comprador</span><input name="buyer"></label><label class="field"><span>Cantidad vendida *</span><input type="number" name="quantity" min="1" max="${Math.max(1,Math.round(Number(product.stock_quantity)||0))}" step="1" value="1" required></label><label class="field"><span>Precio de venta por unidad *</span><input type="number" name="unit_sale_price_ars" min="0.01" step="0.01" value="${h(product.sale_price_ars||'')}" required></label><label class="field"><span>Cotización del dólar *</span><input type="number" name="usd_rate" min="0.01" step="0.01" required></label><label class="field full"><span>Notas</span><textarea name="notes"></textarea></label></div><div class="sale-stock-only-note"><strong>Solo stock</strong><span>Esta acción no genera movimientos en Contabilidad ni reparte importes entre personas.</span></div><button class="btn btn-primary" type="submit">Registrar venta y descontar stock</button></form>`, true);
    }
    ...
    async function renderLaRuda(params) {
        ...
        const viewRaw=params.get('view')||'pedidos';
        const enabledViews=['pedidos','cupones','stock','modelos','publicados','entregados'];
        const view=enabledViews.includes(viewRaw)?viewRaw:'pedidos';
        const summary=data.summary||{},seasons=data.seasons||[],activeSeason=seasons.find(x=>Number(x.is_active))||seasons[0]||null;
        const active=(data.orders||[]).filter(o=>!['entregado','cancelado'].includes(o.status));
        const deliveredAll=(data.orders||[]).filter(o=>o.status==='entregado');
        const deliveredSeasonId=params.has('season_id')?Number(params.get('season_id')||0):Number(activeSeason?.id||0);
        const deliveredQuery=(params.get('q')||'').trim().toLocaleLowerCase('es-AR');
        const delivered=deliveredAll.filter(o=>(!deliveredSeasonId||Number(o.season_id)===deliveredSeasonId)&&(!deliveredQuery||[o.customer_code,o.customer_name,o.customer_contact].some(v=>String(v||'').toLocaleLowerCase('es-AR').includes(deliveredQuery))));
        const orderCards=rows=>rows.length?`<div class="la-ruda-order-grid">${rows.map(o=>{const pct=Number(o.stage_count)?Math.round(Number(o.completed_count)*100/Number(o.stage_count)):0;const previews=o.items_preview||[];return `<button class="la-ruda-order-card status-${h(o.status)}" type="button" data-command="la-ruda-order-open" data-id="${o.id}"><div class="la-ruda-order-card-top"><span class="la-ruda-status status-${h(o.status)}">${h(laRudaStatusLabel(o.status))}</span><strong>#${h(o.customer_code||o.id)}</strong></div><h3>${h(o.customer_name)}</h3><p>${Number(o.item_count||0)} productos · ${moneyARS(o.total_amount)}${Number(o.photo_count||0)?` · ${integerQty(o.photo_count)} fotos`:''}</p><div class="production-progress"><span><i style="width:${pct}%"></i></span><small>${pct}% de etapas</small></div>${previews.length?`<div class="la-ruda-order-tags">${previews.map(item=>`<span>${h(item.product_name)} <strong>× ${integerQty(item.quantity)}</strong></span>`).join('')}</div>`:'<div class="la-ruda-order-tags empty"><span>Sin productos cargados</span></div>'}<footer><span>${formatDate(o.order_date)}</span><span>${o.status==='entregado'&&o.delivered_at?`Entregado ${formatDate(o.delivered_at)}`:(o.due_date?`Entrega ${formatDate(o.due_date)}`:'Sin fecha de entrega')}</span></footer></button>`}).join('')}</div>`:emptyState('◆','Sin pedidos','No hay pedidos en esta vista.');
        ...
        const salesHistory=(data.sales||[]).length?`<section class="panel sales-history-panel"><div class="panel-header"><div><h2>Ventas registradas</h2><p>Historial operativo de ventas y salidas de stock.</p></div></div><div class="stock-history-list">${data.sales.map(s=>`<article class="positive"><time>${formatDate(s.sale_date)}</time><strong>${integerQty(s.quantity)} un.</strong><div><span>${h(s.product_name)} · ${moneyARS(s.total_sale_ars)}</span><small>${s.buyer?`Comprador: ${h(s.buyer)}`:'Salida de stock registrada'}</small></div></article>`).join('')}</div></section>`:'';
        const publishedPage=`${publishedCards}${salesHistory}`;
        const models=data.models||[];
        const modelsPage=`<section class="models-3d-head panel models-3d-head-simple"><div><h2>Modelos 3D</h2><p>Archivos protegidos para descargar, editar o reemplazar cuando haga falta.</p></div><button class="btn btn-primary" data-command="la-ruda-model-new">+ Agregar modelo</button></section>${models.length?`<div class="models-3d-grid">${models.map(m=>`<article class="model-3d-card"><div class="model-3d-file-icon"><strong>${h(String(m.file_extension||'3D').toUpperCase())}</strong><small>${formatFileSize(m.size_bytes)}</small></div><div class="model-3d-copy"><span class="product-line">${h(m.category_name||'Modelo 3D')}</span><h3>${h(m.name)}</h3><p>${h(m.product_name||'Sin producto relacionado')}${m.version_label?` · ${h(m.version_label)}`:''}</p>${m.description?`<small>${h(m.description)}</small>`:''}<footer><span>${formatDateTime(m.created_at)}</span>${m.created_by_name?`<span>${h(m.created_by_name)}</span>`:''}</footer></div><div class="model-3d-actions"><button class="btn btn-small btn-primary" data-command="la-ruda-model-download" data-id="${m.id}" data-name="${h(m.original_name)}">Descargar</button><button class="icon-button" data-command="la-ruda-model-edit" data-id="${m.id}" title="Editar">✎</button><button class="icon-button danger" data-command="la-ruda-model-delete" data-id="${m.id}" title="Eliminar">×</button></div></article>`).join('')}</div>`:emptyState('3D','Todavía no hay modelos','Agregue el primer archivo de impresión o diseño.')}`;
        const couponQuery=(params.get('coupon_q')||'').trim().toLocaleLowerCase('es-AR');
        const coupons=(data.coupons||[]).filter(c=>!couponQuery||[c.customer_code,c.customer_name].some(v=>String(v||'').toLocaleLowerCase('es-AR').includes(couponQuery)));
        const couponTotals={total:(data.coupons||[]).length,used:(data.coupons||[]).filter(c=>Number(c.used)).length};couponTotals.available=couponTotals.total-couponTotals.used;
        const couponsPage=`<section class="la-ruda-coupon-head panel"><div><span class="eyebrow">CUPONES DE CLIENTES</span><h2>Control de cupones</h2><p>El número de cupón es exactamente el mismo número de cliente. Una vez utilizado, el servidor impide registrarlo por segunda vez.</p></div><div class="la-ruda-coupon-stats"><span><small>Total</small><strong>${integerQty(couponTotals.total)}</strong></span><span><small>Disponibles</small><strong>${integerQty(couponTotals.available)}</strong></span><span><small>Utilizados</small><strong>${integerQty(couponTotals.used)}</strong></span></div></section><form class="la-ruda-coupon-search panel" data-form="la-ruda-coupon-filter"><label class="field"><span>Buscar número de cupón o cliente</span><input type="search" name="coupon_q" value="${h(params.get('coupon_q')||'')}" placeholder="Ej.: número de cupón o nombre del cliente" autofocus></label><button class="btn btn-primary" type="submit">Buscar</button><a class="btn btn-ghost" href="#/apiario-la-ruda?view=cupones">Limpiar</a></form>${coupons.length?`<div class="la-ruda-coupon-grid">${coupons.map(c=>`<article class="la-ruda-coupon-card ${Number(c.used)?'used':'available'}"><header><div><small>Número de cupón</small><strong>${h(c.customer_code)}</strong></div><span>${Number(c.used)?'UTILIZADO':'DISPONIBLE'}</span></header><h3>${h(c.customer_name||'Cliente')}</h3><p>${integerQty(c.order_count||0)} pedido${Number(c.order_count||0)===1?'':'s'} · último ${formatDate(c.last_order_date)}</p>${Number(c.used)?`<div class="la-ruda-coupon-used"><strong>✓ Ya fue utilizado</strong><small>${formatDateTime(c.redeemed_at)}${c.redeemed_by_name?` · registrado por ${h(c.redeemed_by_name)}`:''}</small></div>`:`<button type="button" class="btn btn-primary" data-command="la-ruda-coupon-redeem" data-code="${h(c.customer_code)}" data-customer="${h(c.customer_name||'')}">Marcar como utilizado</button>`}</article>`).join('')}</div>`:emptyState('◇','No hay coincidencias','Revise el número de cupón o el nombre del cliente.')}`;
        const deliveredFilters=`<form class="la-ruda-delivered-filter panel" data-form="la-ruda-delivered-filter"><div><span class="eyebrow">HISTORIAL</span><h3>Pedidos entregados</h3><p>La temporada activa se muestra por defecto. Puede volver a cualquier temporada anterior.</p></div><label class="field"><span>Buscar</span><input type="search" name="q" value="${h(params.get('q')||'')}" placeholder="Cliente o LR-..."></label><label class="field"><span>Temporada</span><select name="season_id"><option value="0" ${!deliveredSeasonId?'selected':''}>Todas las temporadas</option>${seasons.map(x=>`<option value="${x.id}" ${Number(x.id)===deliveredSeasonId?'selected':''}>${h(x.name)}${Number(x.is_active)?' · ACTIVA':''}</option>`).join('')}</select></label><div class="la-ruda-delivered-filter-actions"><button class="btn btn-primary" type="submit">Filtrar</button><a class="btn btn-ghost" href="#/apiario-la-ruda?view=entregados${activeSeason?.id?`&season_id=${activeSeason.id}`:''}">Temporada actual</a></div></form>`;
        const page=view==='stock'?stockCards:view==='publicados'?publishedPage:view==='modelos'?modelsPage:view==='cupones'?couponsPage:view==='entregados'?`${deliveredFilters}${orderCards(delivered)}`:orderCards(active);
        shell({title:'Apiario La Ruda',subtitle:'Pedidos, cupones, artículos y entregas',active:'la-ruda',actions:'<button class="btn btn-secondary" data-command="la-ruda-product-new">+ Producto</button><button class="btn btn-primary" data-command="la-ruda-order-new">+ Nuevo pedido</button>',content:`<section class="la-ruda-summary la-ruda-summary-v17"><article><small>Pedidos activos</small><strong>${integerQty(summary.active)}</strong></article><article><small>Cupones disponibles</small><strong>${integerQty(couponTotals.available)}</strong></article><article><small>Artículos publicados</small><strong>${integerQty((data.published||[]).length)}</strong></article><article><small>Entregados</small><strong>${integerQty(deliveredAll.length)}</strong></article></section><nav class="la-ruda-tabs"><a class="${view==='pedidos'?'active':''}" href="#/apiario-la-ruda?view=pedidos">Pedidos activos</a><a class="${view==='cupones'?'active':''}" href="#/apiario-la-ruda?view=cupones">Cupones</a><a class="${view==='stock'?'active':''}" href="#/apiario-la-ruda?view=stock">Stock y productos</a><a class="${view==='modelos'?'active':''}" href="#/apiario-la-ruda?view=modelos">Modelos 3D</a><a class="${view==='publicados'?'active':''}" href="#/apiario-la-ruda?view=publicados">Artículos publicados</a><a class="${view==='entregados'?'active':''}" href="#/apiario-la-ruda?view=entregados">Entregados</a></nav>${page}`});
    }
    ...
    function initAccountingRowViews() {
        document.querySelectorAll('[data-accounting-row]').forEach(row => {
            const open = () => openAccountingView(state.accountingEntries.find(entry => Number(entry.id) === Number(row.dataset.id)));
            row.addEventListener('click', event => { if (!event.target.closest('button,a,input,select,textarea,label')) open(); });
            row.addEventListener('keydown', event => { if ((event.key === 'Enter' || event.key === ' ') && !event.target.closest('button,a,input,select,textarea,label')) { event.preventDefault(); open(); } });
        });
    }
    ...
    function initAccountingCalculation() {
        ...
        const personField = form.querySelector('[data-accounting-person-field]');
        const personSelect = personField?.querySelector('select');
        ...
        const update = () => {
            ...
            if (personField) personField.hidden = automaticIncome;
            if (personSelect) { personSelect.required = !automaticIncome; personSelect.disabled = automaticIncome; }
            if (allocationBox) allocationBox.hidden = !automaticIncome;
            if (automaticIncome && allocationRows) {
                const rows = calculateApiarioIncomeAllocation(ars.value);
    ...
    const technicalTabs = (view, hiveId='', seasonId='') => `<nav class="technical-tabs">${technicalViews.map(([v,l])=>{const q=new URLSearchParams({view:v});if(hiveId)q.set('hive_id',hiveId);if(seasonId)q.set('season_id',seasonId);return `<a class="${view===v?'active':''}" href="#/technical?${q.toString()}">${l}</a>`}).join('')}</nav>`;
    const technicalTypeLabels={tratamiento:'Tratamiento',control_varroa:'Control Varroa',control_nosema:'Control Nosema',loque:'Loque / control de cría',medicacion:'Medicación',diagnostico:'Diagnóstico',otro:'Otro'};
    const technicalRecordHasHive=(item,hiveId)=>!hiveId||String(item.hive_id||'')===String(hiveId)||(item.hives||[]).some(x=>String(x.hive_id||x.id||'')===String(hiveId));
    const technicalHiveName=(hiveId,fallback='')=>(state.apiaryTechnical?.hives||[]).find(x=>String(x.id)===String(hiveId))?.name||fallback||'Colmena';
    const technicalSeasonName=(item={})=>item.season_name||(state.apiaryTechnical?.seasons||[]).find(x=>String(x.id)===String(item.season_id))?.name||'Sin temporada';
    const technicalHiveChips=item=>{const rows=(item.hives||[]).length?item.hives:(item.hive_id?[{hive_id:item.hive_id,hive_name:technicalHiveName(item.hive_id,item.hive_name)}]:[]);return rows.length?`<div class="technical-view-hives">${rows.map(x=>`<span>⬡ ${h(x.hive_name||technicalHiveName(x.hive_id||x.id))}</span>`).join('')}</div>`:'<span class="technical-view-empty">Sin colmenas asociadas</span>'};
    const technicalViewField=(label,value,wide=false)=>`<div class="technical-view-field ${wide?'wide':''}"><small>${h(label)}</small><strong>${value===null||value===undefined||value===''?'—':h(String(value))}</strong></div>`;

    async function openInspectionView(item={}) {
        let files=[];
        if(item.id){try{const d=await api('apiary_inspection_get',{params:{id:item.id}});item={...item,...(d.inspection||{})};files=d.files||[]}catch(_){}}
        const hiveName=technicalHiveName(item.hive_id,item.hive_name);
        const attachmentHtml=files.length?`<section class="technical-view-section"><div class="technical-view-section-title"><span>▤</span><div><small>EVIDENCIA</small><h3>Fotos y archivos</h3></div></div><div class="technical-view-files">${files.map(f=>`<button type="button" class="technical-view-file" data-command="open-file" data-file-type="apiary_inspection" data-id="${f.id}" data-name="${h(f.original_name)}">${String(f.mime_type).startsWith('image/')?`<img data-protected-image data-file-type="apiary_inspection" data-id="${f.id}" alt="${h(f.original_name)}">`:'<span class="technical-view-pdf">PDF</span>'}<strong>${h(f.original_name)}</strong><small>Abrir archivo</small></button>`).join('')}</div></section>`:'';
        showAppModal(`Inspección · ${hiveName}`,`<div class="technical-view technical-view-inspection"><section class="technical-view-hero"><div class="technical-view-icon">◉</div><div><span class="eyebrow">INSPECCIÓN REALIZADA</span><h2>${h(hiveName)}</h2><p>${formatDate(item.inspection_date)} · ${h(technicalSeasonName(item))}</p></div><div class="technical-view-actions"><button class="btn btn-secondary" type="button" data-command="apiary-inspection-edit" data-id="${item.id}">Editar</button><button class="btn btn-ghost" type="button" data-command="modal-close">Cerrar</button></div></section><section class="technical-view-grid">${technicalViewField('Reina vista',Number(item.queen_seen)?'Sí':'No')}${technicalViewField('Postura',capitalize(item.laying_status||'sin evaluar'))}${technicalViewField('Cuadros cubiertos',`${integerQty(item.frames_bees||0)} cuadros`)}${technicalViewField('Reserva de miel',capitalize(item.honey_reserve_status||'sin evaluar'))}${technicalViewField('Reserva de polen',capitalize(item.pollen_reserve_status||'sin evaluar'))}${technicalViewField('Celdas reales',integerQty(item.queen_cells||0))}${technicalViewField('Temperamento',capitalize(item.temperament||'sin evaluar'))}${technicalViewField('Enjambrazón',Number(item.swarm_signs)?'Con signos':'Sin signos')}</section>${item.notes?`<section class="technical-view-notes"><small>OBSERVACIONES</small><p>${nl2br(item.notes)}</p></section>`:''}${attachmentHtml}</div>`);
    }

    function openHealthView(item={}) {
        const type=technicalTypeLabels[item.treatment_type]||capitalize(item.treatment_type||'Registro sanitario');
        showAppModal(`${type} · ${item.condition_name||'Sanidad'}`,`<div class="technical-view technical-view-health"><section class="technical-view-hero"><div class="technical-view-icon">✚</div><div><span class="eyebrow">REGISTRO SANITARIO</span><h2>${h(item.condition_name||type)}</h2><p>${formatDate(item.record_date)} · ${h(technicalSeasonName(item))}</p></div><div class="technical-view-actions"><button class="btn btn-secondary" type="button" data-command="apiary-health-edit" data-id="${item.id}">Editar</button><button class="btn btn-ghost" type="button" data-command="modal-close">Cerrar</button></div></section><section class="technical-view-grid">${technicalViewField('Tipo',type)}${technicalViewField('Producto',item.product||'Sin producto')}${technicalViewField('Dosis',item.dose||'Sin indicar')}${technicalViewField('Finalización',item.end_date?formatDate(item.end_date):'Sin indicar')}${technicalViewField('Resultado',item.result||'Sin indicar')}</section><section class="technical-view-section"><div class="technical-view-section-title"><span>⬡</span><div><small>APLICADO EN</small><h3>Colmenas</h3></div></div>${technicalHiveChips(item)}</section>${item.notes?`<section class="technical-view-notes"><small>OBSERVACIONES</small><p>${nl2br(item.notes)}</p></section>`:''}</div>`);
    }

    function openHarvestView(item={}) {
        showAppModal(`Cosecha · ${item.batch_code||formatDate(item.harvest_date)}`,`<div class="technical-view technical-view-harvest"><section class="technical-view-hero"><div class="technical-view-icon">⬢</div><div><span class="eyebrow">COSECHA REGISTRADA</span><h2>${Number(item.total_kg||0).toLocaleString('es-AR',{maximumFractionDigits:3})} kg</h2><p>${formatDate(item.harvest_date)} · ${h(technicalSeasonName(item))}</p></div><div class="technical-view-actions"><button class="btn btn-secondary" type="button" data-command="apiary-harvest-edit" data-id="${item.id}">Editar</button><button class="btn btn-ghost" type="button" data-command="modal-close">Cerrar</button></div></section><section class="technical-view-grid">${technicalViewField('Lote',item.batch_code||'Sin lote')}${technicalViewField('Tipo de miel',item.honey_type||'Sin indicar')}${technicalViewField('Humedad',item.moisture_pct?`${Number(item.moisture_pct).toLocaleString('es-AR')} %`:'Sin indicar')}${technicalViewField('Recipientes',item.containers||'Sin indicar')}</section><section class="technical-view-section"><div class="technical-view-section-title"><span>⬡</span><div><small>ORIGEN</small><h3>Colmenas participantes</h3></div></div>${technicalHiveChips(item)}</section>${item.notes?`<section class="technical-view-notes"><small>OBSERVACIONES</small><p>${nl2br(item.notes)}</p></section>`:''}</div>`);
    }

    function openFeedingView(item={}) {
        showAppModal(`Alimentación · ${item.feed_type||'Registro'}`,`<div class="technical-view technical-view-feeding"><section class="technical-view-hero"><div class="technical-view-icon">◒</div><div><span class="eyebrow">ALIMENTACIÓN REGISTRADA</span><h2>${h(item.feed_type||'Alimentación')}</h2><p>${formatDate(item.feeding_date)} · ${h(technicalSeasonName(item))}</p></div><div class="technical-view-actions"><button class="btn btn-secondary" type="button" data-command="apiary-feeding-edit" data-id="${item.id}">Editar</button><button class="btn btn-ghost" type="button" data-command="modal-close">Cerrar</button></div></section><section class="technical-view-grid">${technicalViewField('Cantidad por colmena',`${Number(item.quantity_per_hive||0).toLocaleString('es-AR',{maximumFractionDigits:3})} ${item.unit||''}`)}${technicalViewField('Motivo',item.reason||'Sin indicar')}</section><section class="technical-view-section"><div class="technical-view-section-title"><span>⬡</span><div><small>APLICADO EN</small><h3>Colmenas</h3></div></div>${technicalHiveChips(item)}</section>${item.notes?`<section class="technical-view-notes"><small>OBSERVACIONES</small><p>${nl2br(item.notes)}</p></section>`:''}</div>`);
    }
    ...
    async function renderTechnical(params) {
        ...
        if(view==='inspections'){
            const d=await api('apiary_inspections_list',{params:{season_id:seasonFilter}});state.apiaryTechnical={...state.apiaryTechnical,...d};const rows=(d.inspections||[]).filter(x=>technicalRecordHasHive(x,hiveId));actions=`<button class="btn btn-primary" data-command="apiary-inspection-new" ${hiveId?`data-hive-id="${hiveId}"`:''}>+ Nueva inspección</button>`;
            page=`<section class="technical-summary-grid"><article><small>Temporada</small><strong>${h(seasonFilter?((base.seasons||[]).find(x=>String(x.id)===String(seasonFilter))?.name||base.season?.name||'—'):'Todas')}</strong></article><article><small>${focusedHive?'Inspecciones de la colmena':'Inspecciones'}</small><strong>${integerQty(focusedHive?rows.length:(base.summary||{}).inspections||0)}</strong></article><article><small>Cosecha registrada</small><strong>${Number((base.summary||{}).harvest_kg||0).toLocaleString('es-AR',{maximumFractionDigits:2})} kg</strong></article></section><section class="technical-card-grid">${rows.length?rows.map(x=>{const cover=hiveCoverById[String(x.hive_id||'')]||'';return `<button class="technical-record-card inspection-card technical-view-trigger ${cover?'with-thumb':''}" data-command="apiary-inspection-view" data-id="${x.id}"><div class="technical-record-head"><div><span class="eyebrow">${formatDate(x.inspection_date)}</span><h3>${h(x.hive_name)}</h3></div>${cover?`<span class="technical-record-thumb"><img class="protected-image" data-protected-image="1" data-file-type="hive" data-id="${cover}" alt="${h(x.hive_name)}"></span>`:''}</div><div class="inspection-quick-metrics inspection-clear-metrics"><span><small>Reina</small><b>${Number(x.queen_seen)?'Vista':'No vista'}</b></span><span><small>Postura</small><b>${h(capitalize(x.laying_status||'sin evaluar'))}</b></span><span><small>Reservas</small><b>${h(capitalize(x.honey_reserve_status||'sin evaluar'))}</b></span><span><small>Polen</small><b>${h(capitalize(x.pollen_reserve_status||'sin evaluar'))}</b></span></div><div class="technical-card-footer"><small>${x.file_count?`${integerQty(x.file_count)} archivos · `:''}${h(x.created_by_name||'')}</small><b>Ver detalle →</b></div></button>`}).join(''):emptyState('◉',focusedHive?'Sin inspecciones para esta colmena':'Todavía no hay inspecciones','Registre una visita para comenzar el historial técnico.')}</section>`;
        } else if(view==='harvests'){
            const d=await api('apiary_harvests_list',{params:{season_id:seasonFilter}});state.apiaryTechnical={...state.apiaryTechnical,...d};const rows=(d.harvests||[]).filter(x=>technicalRecordHasHive(x,hiveId));actions=`<button class="btn btn-primary" data-command="apiary-harvest-new">+ Registrar cosecha</button>`;
            page=`<section class="technical-card-grid harvest-grid">${rows.length?rows.map(x=>`<button class="technical-record-card harvest-card technical-view-trigger" data-command="apiary-harvest-view" data-id="${x.id}"><span class="eyebrow">${formatDate(x.harvest_date)} · ${h(x.season_name||'')}</span><div class="harvest-total"><strong>${Number(x.total_kg||0).toLocaleString('es-AR',{maximumFractionDigits:3})}</strong><span>kg</span></div><h3>${h(x.batch_code||'Cosecha sin lote')}</h3><p>${h(x.honey_type||'Tipo de miel sin indicar')} · ${integerQty(x.hive_count||0)} colmenas</p><div class="technical-card-footer"><small>${x.moisture_pct?`Humedad ${Number(x.moisture_pct).toLocaleString('es-AR')}%`:''}</small><b>Ver detalle →</b></div></button>`).join(''):emptyState('⬢',focusedHive?'Sin cosechas para esta colmena':'No hay cosechas registradas','Registre una extracción para comenzar la trazabilidad de producción.')}</section>`;
        } else if(view==='health'){
            const d=await api('apiary_health_list',{params:{season_id:seasonFilter}});state.apiaryTechnical={...state.apiaryTechnical,...d};const rows=(d.records||[]).filter(x=>technicalRecordHasHive(x,hiveId));actions=`<button class="btn btn-primary" data-command="apiary-health-new" ${hiveId?`data-hive-id="${hiveId}"`:''}>+ Registro sanitario</button>`;
            page=`<section class="technical-card-grid">${rows.length?rows.map(x=>`<button class="technical-record-card health-card technical-view-trigger" data-command="apiary-health-view" data-id="${x.id}"><span class="eyebrow">${formatDate(x.record_date)} · ${h(technicalTypeLabels[x.treatment_type]||capitalize(x.treatment_type))}</span><h3>${h(x.condition_name||capitalize(x.treatment_type))}</h3><p>${h(x.product||'Sin producto')} ${x.dose?`· ${h(x.dose)}`:''}</p><div class="technical-chip-row">${(x.hives||[]).slice(0,4).map(v=>`<span>${h(v.hive_name)}</span>`).join('')}${Number(x.hive_count)>4?`<span>+${Number(x.hive_count)-4}</span>`:''}</div><div class="technical-card-footer"><small>${x.result?`Resultado: ${h(x.result)}`:'Sin resultado informado'}</small><b>Ver detalle →</b></div></button>`).join(''):emptyState('✚',focusedHive?'Sin tratamientos ni controles para esta colmena':'No hay registros sanitarios','Los tratamientos y controles quedarán asociados a cada colmena.')}</section>`;
        } else if(view==='feeding'){
            const d=await api('apiary_feedings_list',{params:{season_id:seasonFilter}});state.apiaryTechnical={...state.apiaryTechnical,...d};const rows=(d.feedings||[]).filter(x=>technicalRecordHasHive(x,hiveId));actions=`<button class="btn btn-primary" data-command="apiary-feeding-new" ${hiveId?`data-hive-id="${hiveId}"`:''}>+ Registrar alimentación</button>`;
            page=`<section class="technical-card-grid">${rows.length?rows.map(x=>`<button class="technical-record-card feeding-card technical-view-trigger" data-command="apiary-feeding-view" data-id="${x.id}"><span class="eyebrow">${formatDate(x.feeding_date)}</span><h3>${h(x.feed_type)}</h3><div class="feeding-amount"><strong>${Number(x.quantity_per_hive||0).toLocaleString('es-AR',{maximumFractionDigits:3})}</strong><span>${h(x.unit)} / colmena</span></div><p>${integerQty(x.hive_count||0)} colmenas${x.reason?` · ${h(x.reason)}`:''}</p><div class="technical-card-footer"><small>${h(x.notes||'')}</small><b>Ver detalle →</b></div></button>`).join(''):emptyState('◒',focusedHive?'Sin alimentaciones para esta colmena':'No hay alimentaciones','Registre jarabe, fondant o suplemento para conservar el historial.')}</section>`;
        } else if(view==='performance'){
            const d=await api('apiary_performance',{params:{season_id:seasonFilter}});state.apiaryTechnical={...state.apiaryTechnical,...d};const rows=(d.rows||[]).filter(x=>!hiveId||String(x.id)===String(hiveId));
            page=`<section class="performance-hero panel"><div><span class="eyebrow">TEMPORADA ${h(d.season?.name||'')}</span><h2>${focusedHive?`Rendimiento · ${h(focusedHive.name)}`:'Comparación por colmena'}</h2><p>Producción, inspecciones, sanidad y alimentación sin inventar un puntaje: cada indicador queda visible por separado.</p></div><div class="performance-summary"><article><small>Miel cosechada</small><strong>${Number(focusedHive?(rows[0]?.harvest_kg||0):(d.summary?.harvest_kg||0)).toLocaleString('es-AR',{maximumFractionDigits:2})} kg</strong></article><article><small>Inspecciones</small><strong>${integerQty(focusedHive?(rows[0]?.inspections||0):(d.summary?.inspections||0))}</strong></article></div></section>${rows.length?`<div class="table-wrap panel"><table class="data-table performance-table"><thead><tr><th>Colmena</th><th>Miel</th><th>Inspecciones</th><th>Abejas prom.</th><th>Reserva miel</th><th>Reserva polen</th><th>Sanidad</th><th>Alimentación</th><th>Reinas</th></tr></thead><tbody>${rows.map(x=>`<tr><td><a href="#/hive/${x.id}"><strong>${h(x.name)}</strong></a><small>${x.last_inspection?`Última inspección ${formatDate(x.last_inspection)}`:'Sin inspecciones'}</small></td><td><strong>${Number(x.harvest_kg||0).toLocaleString('es-AR',{maximumFractionDigits:2})} kg</strong></td><td>${integerQty(x.inspections||0)}</td><td>${Number(x.avg_bees||0).toLocaleString('es-AR',{maximumFractionDigits:1})} c.</td><td>${h(capitalize(x.latest_honey_reserve||'—'))}</td><td>${h(capitalize(x.latest_pollen_reserve||'—'))}</td><td>${integerQty(x.health_events||0)}</td><td>${integerQty(x.feeding_events||0)}</td><td>${integerQty(x.queen_changes||0)}</td></tr>`).join('')}</tbody></table></div>`:emptyState('◫','Sin datos comparables','Las métricas aparecerán cuando registre inspecciones, cosechas, sanidad o alimentación.')}`;
        } else {
    ...
    document.addEventListener('change', event => {
        ...
        if (seasonFilter) go('/technical', { view: seasonFilter.dataset.view, season_id: seasonFilter.value, hive_id: seasonFilter.dataset.hiveId || '' });
    });
    ...
    document.addEventListener('submit', async event => {
        ...
        try {
            if (type === 'login') {
                ...
                const result = await api('activity_save', { method: 'POST', formData: new FormData(form) }); invalidateImageCache('activity'); toast(result.message); if (form.closest('.app-modal')) { const currentPath=parse().path; closeAppModal(); if(currentPath.startsWith('/hive/')) await route(); else go('/activities'); } else go(`/activity-edit/${result.id}`);
                ...
                const orderId=Number(form.elements.order_id.value);await api('la_ruda_item_save',{method:'POST',data:Object.fromEntries(new FormData(form).entries())});toast('Producto agregado');await openLaRudaOrder(orderId,true);
            } else if (type === 'la-ruda-order-photo-upload') {
                const orderId=Number(form.elements.order_id.value);const result=await api('la_ruda_order_photo_upload',{method:'POST',formData:new FormData(form)});invalidateImageCache('la_ruda_order_photo');toast(result.message||'Foto guardada');await openLaRudaOrder(orderId,true);
            } else if (type === 'la-ruda-coupon-filter') {
                const values=Object.fromEntries(new FormData(form).entries());go('/apiario-la-ruda',{view:'cupones',coupon_q:values.coupon_q||''});
            } else if (type === 'la-ruda-delivered-filter') {
                const values=Object.fromEntries(new FormData(form).entries());go('/apiario-la-ruda',{view:'entregados',season_id:values.season_id||0,q:values.q||''});
    ...
    document.addEventListener('click', async event => {
        ...
        try {
            if (command === 'app-switch-toggle') {
            ...
            } else if (command === 'apiary-inspection-view') {
                const row=(state.apiaryTechnical.inspections||[]).find(x=>String(x.id)===String(commandElement.dataset.id));if(row)await openInspectionView(row);
                ...
                openHarvestModal();
            } else if (command === 'apiary-harvest-view') {
                const row=(state.apiaryTechnical.harvests||[]).find(x=>String(x.id)===String(commandElement.dataset.id));if(row)openHarvestView(row);
            ...
            } else if (command === 'apiary-health-view') {
                const row=(state.apiaryTechnical.records||[]).find(x=>String(x.id)===String(commandElement.dataset.id));if(row)openHealthView(row);
            ...
            } else if (command === 'apiary-feeding-view') {
                const row=(state.apiaryTechnical.feedings||[]).find(x=>String(x.id)===String(commandElement.dataset.id));if(row)openFeedingView(row);
                ...
                await openLaRudaOrder(Number(commandElement.dataset.id),false);
            } else if (command === 'la-ruda-order-edit-mode') {
                await openLaRudaOrder(Number(commandElement.dataset.id),true);
            } else if (command === 'la-ruda-order-view-mode') {
                await openLaRudaOrder(Number(commandElement.dataset.id),false);
                ...
                await api('la_ruda_order_status',{method:'POST',data:{id:commandElement.dataset.id,status:commandElement.dataset.status}});toast('Estado actualizado');await openLaRudaOrder(Number(commandElement.dataset.id),true);
            } else if (command === 'la-ruda-stage-toggle') {
                await api('la_ruda_stage_toggle',{method:'POST',data:{id:commandElement.dataset.id,completed:commandElement.dataset.completed}});toast('Etapa actualizada');await openLaRudaOrder(Number(commandElement.dataset.orderId),true);
            } else if (command === 'la-ruda-item-delete') {
                if(confirm('¿Quitar este producto del pedido?')){await api('la_ruda_item_delete',{method:'POST',data:{id:commandElement.dataset.id}});await openLaRudaOrder(Number(commandElement.dataset.orderId),true);}
            } else if (command === 'la-ruda-order-photo-delete') {
                if(confirm('¿Eliminar esta foto del pedido?')){await api('la_ruda_order_photo_delete',{method:'POST',data:{id:commandElement.dataset.id}});invalidateImageCache('la_ruda_order_photo');toast('Foto eliminada');await openLaRudaOrder(Number(commandElement.dataset.orderId),true);}
            } else if (command === 'la-ruda-coupon-redeem') {
                const code=commandElement.dataset.code||'';const customer=commandElement.dataset.customer||'';
                if(confirm(`¿Marcar el cupón ${code}${customer?` de ${customer}`:''} como UTILIZADO?\n\nDespués no podrá registrarse un segundo uso.`)){const r=await api('la_ruda_coupon_redeem',{method:'POST',data:{customer_code:code}});toast(r.message||'Cupón utilizado');await route();}
                ...
                const product=(state.laRudaData?.products||[]).find(x=>String(x.id)===String(commandElement.dataset.id));if(product)await openLaRudaSale(product);
                ...
                const activityId = Number(commandElement.dataset.id || 0);
                if (activityId) await openActivityView(activityId);
                else await openActivityModal(0, { hiveId: commandElement.dataset.hiveId || '' });
            } else if (command === 'activity-edit') {
                await openActivityModal(Number(commandElement.dataset.id || 0));
